...
class MapSerializer extends SerializerBase
{
    ...
    public function WriteObject($obj, $outStream, $etype = null)
    {
        ...
        foreach ($obj as $key => $value) {
            ...
            SerializerFactory::GetSerializer($keyTypeId)->WriteObject($key, $outStream, $keyElem);
            ...
            if (null === $value) {
                ...
                SerializerFactory::GetSerializer($valueTypeId)->WriteObject($value, $outStream, $valueElem);
            }
        }
    }
    ...
    public function ReadObject($inStream, $defType, $etype = null)
    {
        ...
        for ($i = 0; $i < $len; $i ++) {
            ...
            $key = SerializerFactory::GetSerializer($keyTypeId)->ReadObject($inStream, $keyType);

            $valueTypeId = $inStream->ReadInt32();
            $valueType = TypeHelper::GetType($valueTypeId);
            if ($valueType !== null) {
                $value = SerializerFactory::GetSerializer($valueTypeId)->ReadObject($inStream, $valueType);
            }
            $tempArray[$key] = $value;
        }
