...
class ProtocolHelper
{
     ...
     * @throws Exception
     */
    public function pV1FromBytes ($data)
    {
        try {
            $p = new Protocol();
            ...
            $p->setSdpEntity(SerializeBase::getInstance($st)->deserialize($sdpData, SDPTypeFactory::getSDPClass($p)));
            return $p;
        } catch (\Exception $e) {
            throw $e;
        }
     ...
     * byte[] desKey
     */
    public function pV2FromBytes($data)
    {
        try {
            $p = new Protocol();
            ...
            $p->setTiresiasData($serialize->deserialize($tiresiasData, SCFType::STRING));
            $p->setKey($serialize->deserialize($keyData, SCFType::STRING));
            $p->setSdpEntity($serialize->deserialize($sdpData, SDPTypeFactory::getSDPClass($p)));
            return $p;
        } catch (\Exception $e) {
            throw $e;
        }
    ...
    public function pV1ToBytes($p)
    {
        try {
            ...
            $sdpData = $serialize->serialize($p->getSdpEntity());
    ...
    public function pV2ToBytes($p)
    {
        try {
            ...
            $p->setSDPType($p->getSdpType());
            $sdpData = $serialize->serialize($p->getSdpEntity());
            $tiresiasData = $serialize->serialize($p->getTiresiasData());
            $keyData = $serialize->serialize($p->getKey());
