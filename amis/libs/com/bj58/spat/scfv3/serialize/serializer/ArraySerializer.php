...
class ArraySerializer extends SerializerBase
{
    ...
    public function WriteObject($obj, $outStream, $etype = null)
    {
        ...
        if ($typeId == SCFType::OBJECT) {
            $elem = $etype['class'];
            $typeId = ObjectSerializer::GetTypeInfo($elem, $initKeyObj)->TypeId;
        }
    ...
    public function ReadObject($inStream, $defType, $etype = null)
    {
        ...
        for ($i = 0; $i < $len; $i ++) {
            ...
            if ($itemTypeId === 0) {
                ...
                $value = SerializerFactory::GetSerializer($itemTypeId, $initKeyObj)->ReadObject($inStream, $defType);
                $tempArray[] = $value;
            }
        }
