...
class SF_NodeState {
    ...
    public function down() {
    	...
    	if($accessed && $now - $accessed > $this->fail_timeout) {
    		$this->resume();
    		LogHelper::logWarnMsg("resume node " . $this->host);
    	}
    	return $this->down;
    }
    ...
    public function fail($num = 1) {
        ...
        LogHelper::logWarnMsg(__CLASS__ . ":". __LINE__. ":". $this->host . " this server fail, decrease weight. current weight is " . $this->weight . 'this server alread fail ' . $this->fails);
    }
