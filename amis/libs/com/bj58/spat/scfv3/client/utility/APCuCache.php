...
class APCuCache{
	...
	public function __construct($srvName) {
		...
		if( !file_exists(self::LOCK_DIR)) {
			mkdir(self::LOCK_DIR);
			chmod(self::LOCK_DIR, self::UMASK);
		}
	}
	/**  Destructor
	 */
	...
	private function getLockFile($key) {
		$srvName = str_replace('/', '-', $this->srvName);

		return self::LOCK_DIR."/". $srvName ."/".$key.".lock";
	}
	...
	public function get($key, &$value) {
		...
		if(false === ($v = @\apcu_fetch($s_key))) {
			...
			if($v['timestamp'] != 0) {
			    ...
			    $arr = array('value' => $value, 'timestamp' => $timestamp);
			    @\apcu_store($s_key, $arr, 0);
			    return true;
			}
			$apcu_ttl = (int)(($v['timestamp'] - $timestamp)/1000);
			if($apcu_ttl > 0) {
			    $arr = array('value' => $value, 'timestamp' => ($timestamp + ($apcu_ttl * 1000)));
			    @\apcu_store($s_key, $arr, $apcu_ttl);
			}
			return true;
		}else {
		    if (is_array($v) && array_key_exists('value', $v)) {
		        ...
		        if ($currentTime > $timestamp) {
		            LogHelper::logWarnMsg(__CLASS__ . ":" . __LINE__ ."apcu del config. interval " . ($currentTime - $timestamp));
		            apcu_delete($s_key);
		            return false;
		        }
		    } else {
	...
	public function put($key, $value, $ttl=0){
		...
		if($apcu_ttl != 0) {
		    $v = array();
		    $v['value'] = $value;
		    $v['timestamp'] = (int)(microtime(true) * 1000) + $ttl;
		    @\apcu_store($s_key, $v, $apcu_ttl);
		}
		$timestamp = 0;
		if($ttl > 0) {
