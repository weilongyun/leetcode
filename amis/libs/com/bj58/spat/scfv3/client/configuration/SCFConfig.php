...
class SCFConfig
{
    ...
    public static function loadConfigByScfKey($scfKeyPath, $serviceName, $time, &$scfKeyPathMd5)
    {
        try {
            $retry = 3;
            ...
            while ($retry) {
                ...
                if (null != $keyContent) {
                    ...
                    try {
                        ...
                        if (null!= $ssmData && $ssmData['configChanged']) {
                            $configStr = $ssmData['config'];
                            if (null != $configStr) {
                                $value = self::getXMLStrConfig($scfKeyPath, $configStr);
                                $configCache->put($key, $value[$serviceName], 60000);
                            } else {
                                $errstr = SSMHelper::getErrorMsg($ssmData, $keyContent);
                                throw new ScfException($errstr);
                            }
                        } else {
                            if (null == $configData) {
                                $configCache->unWLock($key);
