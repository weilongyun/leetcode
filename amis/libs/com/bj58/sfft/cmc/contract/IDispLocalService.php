...
class IDispLocalService {
	...
	public function GetDispLocalByID($dispLocalID = '') {
		ObjectSerializer::GetTypeInfo(new DispLocal());
		$value = array(
			'1@int' => $dispLocalID,
		);
	...
	public function ReCacheDispLocal() {
		$value = array(
		);
		$res = $this->ps->invoke('ReCacheDispLocal', $value);
		return $res;
	}
